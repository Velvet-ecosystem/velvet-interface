...
- `private` preserves the owner-private/boudoir visual lineage. It is explicitly separate from Backroom and the owner-maintenance hierarchy known as Velvet's Legs. It remains deferred until its owner/private access and safe-state rules are implemented deliberately.
...
## Backroom versus Velvet's Legs

The Front Room has a discreet Backroom seam. Backroom is the technical/read-only diagnostics room.

The deeper Velvet's Legs entrance is different: its Backroom press point emits a local presentation event and application code requires both owner-presence and protected-maintenance evidence before opening the trusted built-in scene. No manifest directly navigates to `velvets_legs`.
